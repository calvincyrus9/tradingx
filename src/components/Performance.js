          ...
          <div className="bg-gray-800/50 backdrop-blur-md border border-gray-700 rounded-xl p-6">
            ...
            <div className="mt-6 overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-700">
                <thead>
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-medium text-cyan-400 uppercase tracking-wider">Market</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-cyan-400 uppercase tracking-wider">PNL</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-cyan-400 uppercase tracking-wider">Max DD</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-cyan-400 uppercase tracking-wider">Trades</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-cyan-400 uppercase tracking-wider">Win Rate</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-cyan-400 uppercase tracking-wider">PF</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-800">
                  {performanceData.map((item, index) => (
                    <tr key={index} className={index % 2 === 0 ? 'bg-gray-900/50' : ''}>
                      <td className="px-4 py-3 whitespace-nowrap font-medium">{item.market}</td>
                      <td className={`px-4 py-3 whitespace-nowrap ${parseFloat(item.pnl) > 5 ? 'text-green-400' : 'text-cyan-400'}`}>
                        {item.pnl}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap text-red-400">{item.maxDrawdown}</td>
                      <td className="px-4 py-3 whitespace-nowrap">{item.trades}</td>
